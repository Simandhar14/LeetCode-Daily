...
class Solution {
public:
    void bfs(unordered_map<int, vector<int>>& adj, vector<bool>& secretPeople,
             int u, unordered_set<int>& visited) {
    ...
    vector<int> findAllPeople(int n, vector<vector<int>>& meetings,
        ...
        for (auto& it : mp) {
            ...
            for (auto& p : it.second) {
                if (secretPeople[p.first] && !visited.count(p.first))
                    bfs(adj, secretPeople, p.first, visited);
                if (secretPeople[p.second] && !visited.count(p.second))
                    bfs(adj, secretPeople, p.second, visited);
            }
        }
